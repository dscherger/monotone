...
typedef map<attr_name, attr_val> attrmap_t;
...
path_vec_t
{
  ...
  bool operator==(path_vec_t const & other) const
  {
    return dir == other.dir && leaf == other.leaf;
  }
...
node
{
  ...
  {
    attrmap_t::const_iterator i = attrs.find(name);
    I(i != attrs.end());
    return i->second;
  }
...
ostream &
operator<<(ostream & o, attrmap_t const & attrs)
{
  for (attrmap_t::const_iterator i = attrs.begin();
       i != attrs.end(); ++i)
    {
      if (i != attrs.begin())
        o << ", ";
  ...
  : public node
{
  ...
  file_node()
  {
    // L(F("constructed file_node %x\n") % this);
  }

  virtual node_t shallow_copy() const;
  virtual ~file_node()
  {
    // L(F("destructing file_node %x\n") % this);
  }
};
...
operator==(weak_node_t const & a,
           weak_node_t const & b)
{
  return a.lock().get() == b.lock().get() && !(a < b || b < a);
}


bool
operator==(node_t const & a,
           node_t const & b)
{
  return a.get() == b.get() && !(a < b || b < a);
}


bool
operator==(dir_t const & a,
           dir_t const & b)
{
  return a.get() == b.get() && !(a < b || b < a);
}
...
mfest::check_sane() const
{
  ...
  hash_set<node_t> seen;
  for(bfs_iter i(root); !i.finished(); ++i)
    {
      I(seen.find(*i) == seen.end());
      seen.insert(*i);
    }
}
...
change_consumer
{
  ...
  virtual void delete_file(path_vec_t const & path,
                           file_id const & ident,
                           attrmap_t const & attrs) = 0;
  virtual void delete_dir(path_vec_t const & path,
                          attrmap_t const & attrs) = 0;
...
{
  L(F("delete_dir('%s', [%s])\n") % dp % attrs);
  this->delete_dir(path_vec_t::from_file_path(dp), attrs);
}
...
{
  I(!ident.inner()().empty());
  L(F("delete_file('%s', [%s], [%s])\n") % fp % ident % attrs);
  this->delete_file(path_vec_t::from_file_path(fp), ident, attrs);
}
...
{
  L(F("add_dir('%s', [%s])\n") % dp % attrs);
  this->add_dir(path_vec_t::from_file_path(dp), attrs);
}
                          ...
                          attrmap_t const & attrs)
{
  L(F("add_file('%s', [%s], [%s])\n") % fp % ident % attrs);
  I(!ident.inner()().empty());
  this->add_file(path_vec_t::from_file_path(fp), ident, attrs);
}
...
{
  bool
  operator()(pair<path_vec_t, node_t> const & a,
             pair<path_vec_t, node_t> const & b) const
  {
  ...
  for (dfs_iter i(new_mfest.root); !i.finished(); ++i)
    {
      ...
      i.leaf(entry);
      // i.path(pv);
      // L(F("sanity pass 2: checking %s\n") % pv.to_file_path());
      get_new_parent(*i, indexed_parent, indexed_entry);
      I(entry == indexed_entry);

      // NB: in the *new* set, it is *not* an invariant
      // that the parent is the same as the indexed parent;
      // the indexed parent may be the old one if there was
      // no copy made.
      //
      // I(parent == indexed_parent);
    }
...
cset::update_old_new_mapping(weak_node_t old, weak_node_t modified_new)
{
  ...
  // L(F("re-associating old %x -> new %x\n")
  //     % old.lock().get()
...
node_t
cset::form_new_node(node_t n)
{
  // L(F("forming new node from shallow copy\n"));
  weak_node_t old_weak;
  get_corresponding_old_weak_node(n, old_weak);
  node_t result = n->shallow_copy();
  update_old_new_mapping(old_weak, weak_node_t(result));
  if (is_dir_t(result))
    {
      dir_t d = downcast_to_dir_t(result);
      for (dirmap_t::const_iterator i = d->entries.begin();
           i != d->entries.end(); ++i)
        {
          set_new_parent(i->second, d, i->first);
        }
    }
  I(result.unique());
  // L(F("formed new node from shallow copy\n"));
  return result;
}
...
cset::unique_entry_in_new_dir(dir_t new_dir, dirent_t entry)
{
  ...
  node_t result;
  //   L(F("unique_entry_in_new_dir(new_dir=%x, entry='%s'): old node=%x refcount=%d\n")
  //     % new_dir.get()
  //     % entry
  //     % e->second.get()
  //     % e->second.use_count());
  ...
  if (e->second.unique())
    ...
    {
      result = form_new_node(e->second);
      new_dir->entries[entry] = result;
      //       L(F("unique_entry_in_new_dir(new_dir=%x, entry='%s'): setting result=%x parent to new_dir=%x\n")
      //         % new_dir.get() % entry % result.get() % new_dir.get());
...
cset::get_writable_dir_containing(path_vec_t const & pth)
{
  ...
  // L(F("get_writable_dir_containing(%s)\n") % pth.to_file_path());

  if (!new_mfest.root.unique())
    {
      // L(F("get_writable_dir_containing(%s): moving new mfest forwards\n") % pth.to_file_path());
      new_mfest.root = downcast_to_dir_t(form_new_node(new_mfest.root));
    }

  I(new_mfest.root.unique());

  //   L(F("get_writable_dir_containing(%s): uniqifying %d dir components\n")
  //     % pth.to_file_path()
  //     % pth.dir.size());
  ...
  for (vector<dirent_t>::const_iterator i = pth.dir.begin();
       i != pth.dir.end(); ++i)
    {
      //       L(F("get_writable_dir_containing(%s): uniqifying dir component: %s\n")
      //         % pth.to_file_path()
      //         % *i);
      node_t n = unique_entry_in_new_dir(d,*i);
     d = downcast_to_dir_t(n);
    }

  //   L(F("get_writable_dir_containing(%s): uniqified %d dir components\n")
  //     % pth.to_file_path()
  //     % pth.dir.size());
...
{
  // L(F("attach: adding child %x at path '%s'") % n.get() % path.to_file_path());
  dir_t new_dir = get_writable_dir_containing(path);
  // L(F("attach: got writable dir %x") % new_dir.get());
              ...
              attrmap_t const & attrs)
{
  ...
  // L(F("added dir mapping old %x -> new %x\n")
  //     % old_dir.get()
  //     % new_dir.get());
               ...
               attrmap_t const & attrs)
{
  ...
  //   L(F("added file mapping old %x -> new %x\n")
  //     % old_file.get()
  //     % new_file.get());
...
struct replay_record
{
  ...
  struct path_with_attrs
  {
    path_vec_t path;
    attrmap_t attrs;
    bool operator<(path_with_attrs const & other) const
    {
      return path < other.path;
    }
  };

  struct path_with_attrs_and_content
  {
    path_vec_t path;
    attrmap_t attrs;
    file_id content;
    bool operator<(path_with_attrs_and_content const & other) const
    {
      return path < other.path;
    }
  };

  set<path_with_attrs_and_content> files_deleted;
  set<path_with_attrs> dirs_deleted;
  vector<live_paths> nodes_renamed;
  set<path_with_attrs> dirs_added;
  set<path_with_attrs_and_content> files_added;
...
play_back_replay_record(replay_record const & rr,
			change_consumer & cc)
{
  for (set<replay_record::path_with_attrs_and_content>::reverse_iterator i
         = rr.files_deleted.rbegin(); i != rr.files_deleted.rend(); ++i)
    cc.delete_file(i->path, i->content, i->attrs);

  for (set<replay_record::path_with_attrs>::reverse_iterator i = rr.dirs_deleted.rbegin();
       i != rr.dirs_deleted.rend(); ++i)
    cc.delete_dir(i->path, i->attrs);
  ...
  for (set<replay_record::path_with_attrs>::const_iterator i = rr.dirs_added.begin();
       i != rr.dirs_added.end(); ++i)
    cc.add_dir(i->path, i->attrs);

  for (set<replay_record::path_with_attrs_and_content>::const_iterator i
         = rr.files_added.begin(); i != rr.files_added.end(); ++i)
    cc.add_file(i->path, i->content, i->attrs);
				...
				change_consumer & cc)
{
  ...
  for (set<replay_record::path_with_attrs>::reverse_iterator i = rr.dirs_added.rbegin();
  ...
  for (set<replay_record::path_with_attrs>::const_iterator i = rr.dirs_deleted.begin();
		    ...
		    replay_record & rr)
{
  ...
  for (dfs_iter i(cs.new_mfest.root); !i.finished(); ++i)
          ...
          && cs.is_live(other))
	{
      ...
      // attrs which were changed on live nodes
      if (cs.is_live(self)
          && cs.is_live(other)
          && (self->attrs != other->attrs))
	{
	  ...
	  for (attrmap_t::const_iterator a = self->attrs.begin();
	       a != self->attrs.end(); ++a)
	    {
	      attr_val oldval;

	      attrmap_t::const_iterator b = other->attrs.find(a->first);
	  ...
	  for (attrmap_t::const_iterator b = other->attrs.begin();
	       b != other->attrs.end(); ++b)
	    {
	      attr_val newval;

	      attrmap_t::const_iterator a = self->attrs.find(b->first);
	      if (a != self->attrs.end())
		newval = a->second;
  ...
  while (parser.symp())
    {
	  ...
	  parser.hex(t2);
          while (parser.symp(syms::attr))
            {
              parser.sym();
              parser.str(k);
              parser.str(v);
              attrs.insert(make_pair(k,v));
            }
      ...
      else if (parser.symp(syms::delete_dir))
        {
          ...
          cc.delete_dir(file_path(t1), attrs);          
        }
                          ...
                          attrmap_t const & attrs)
{
  ...
  for (attrmap_t::const_iterator i = attrs.begin();
       i != attrs.end(); ++i)
    st.push_str_triple (syms::attr, i->first, i->second);
  ...
  while (p.symp())
          ...
          while (p.symp(syms::attr))
            {
              ...
              p.str(n);
              p.str(v);
              attrs.insert(make_pair(n,v));
            }
          // L(F("read dir %s\n") % pth);
	  cs.add_dir(file_path(pth), attrs);
	}
      else if (p.symp(syms::file))
	{
	  ...
	  p.esym(syms::content);
          ...
          // L(F("read file %s\n") % pth);
	  cs.add_file(file_path(pth), file_id(ident), attrs);
	}
      else break;
    }
	    ...
	    mfest const & m)
{
  for (dfs_iter i(m.root); !i.finished(); ++i)
      ...
      else
	{
      ...
      for (attrmap_t::const_iterator j = curr->attrs.begin();
	   j != curr->attrs.end(); ++j)
	{
	  I(!j->second.empty());
...
change_automaton
{
  ...
  {
    attrmap_t tmp;    
    while (tmp.size() < 3 && !flip(3))
      {
        tmp.insert(make_pair(new_word(), new_word()));
      }
    return tmp;
  }
  ...
  attr_name pick_attr(attrmap_t const & attrs)
  {
  ...
  bool parent_of(path_vec_t const & p,
                 path_vec_t const & c)
  {
    vector<dirent_t> pv, cv;
    copy(p.dir.begin(), p.dir.end(), back_inserter(pv));
    pv.push_back(p.leaf);

    copy(c.dir.begin(), c.dir.end(), back_inserter(cv));
    cv.push_back(c.leaf);
    ...
    if (pv.size() <= cv.size())
      {
        vector<dirent_t>::const_iterator c_anchor = 
          search(cv.begin(), cv.end(),
                 pv.begin(), pv.end());
        
        is_parent = (c_anchor == cv.begin());
      }
  ...
  bool pick_random_live_undamaged_nonroot_paths(cset & c, 
                                                bool & is_dir, 
                                                bool & dir_is_empty, 
                                                path_vec_t & old_path, 
                                                path_vec_t & new_path, 
  ...
  {
    size_t count = 0;
    for (dfs_iter i(c.old_mfest.root); !i.finished(); ++i)
      ++count;
        ...
        if (choice-- == 0)
          {
            node_t n(*i), new_n;
            I(c.is_live(n));
            c.get_corresponding_new_node(weak_node_t(n), new_n);

            if (!c.is_live(new_n))
              return false;

            if (n->attrs != new_n->attrs)
              return false;
            ...
            if (is_file_t(n))
              ...
              {
                is_dir = true;
                dir_is_empty = downcast_to_dir_t(new_n)->entries.empty();
              }

            i.path(old_path);
            c.get_new_path(new_n, new_path);
            attrs = n->attrs;
            return true;
          }
      }
  ...
  void perform_random_action(cset & cs)
  {
    change_consumer & c = cs;

    bool did_something = false;
    while (!did_something)
      {
        bool is_dir, new_dir_is_empty;
        path_vec_t old_path, new_path;
        attrmap_t attrs;
        file_id content;
        if (!pick_random_live_undamaged_nonroot_paths(cs, is_dir, new_dir_is_empty,
                                                      old_path, new_path, 
                                                      attrs, content))
          {
            // Must add, couldn't find anything to work with
            if (flip())
              c.add_dir(file_path(new_word()), new_attrs());
            else
              c.add_file(file_path(new_word()), 
                         file_id(new_ident()),
                         new_attrs());
            did_something = true;
          }
            ...
            switch (rand() % 7)
              ...
              case 2:
                if (is_dir && flip())
                  ...
                  new_path.leaf = new_entry();

                if (flip())
                  c.add_dir(new_path.to_file_path(), new_attrs());
                else
                  c.add_file(new_path.to_file_path(), new_ident(), new_attrs());
                did_something = true;
                break;
              ...
              case 4:
                {
                  bool target_is_dir, target_dir_is_empty;
                  path_vec_t target_old_path, target_new_path;
                  attrmap_t a2;
                  file_id f2;
                  if (pick_random_live_undamaged_nonroot_paths(cs, target_is_dir, target_dir_is_empty,
                                                               target_old_path, target_new_path, 
                                                               a2, f2))
                    {                      
                      if (target_is_dir && flip())
                        target_new_path /= new_entry();
                      else
                        target_new_path.leaf = new_entry();
                      
                      if (!(is_dir && parent_of(new_path, target_new_path)))
                        {
                          c.rename_node(old_path.to_file_path(), 
                                        target_new_path.to_file_path());
                          c.finalize_renames();
                          did_something = true;
                        }
                    }
                ...
                if (is_dir)
                  {
                    if (new_dir_is_empty)
                      {
                        c.delete_dir(old_path.to_file_path(), attrs);
                        did_something = true;
                      }
                  }
                else
                  {
                    c.delete_file(old_path.to_file_path(), content, attrs);
                    did_something = true;
                  }
                break;
              ...
              case 6:
                if (!attrs.empty() && flip())
                  c.clear_attr(new_path.to_file_path(), pick_attr(attrs));
                else
                  c.set_attr(new_path.to_file_path(), new_word(), new_word());
                did_something = true;
                break;                
              }
          }
...
automaton_cset_test()
{
  ...
  for (int i = 0; i < 1000; ++i)
    {
      data tmp;
      //L(F("automaton cycle %d, changeset 1\n") % i);
      cset cs1(m1);
  ...
  try
    {
      ...
      attrmap_t ea;
      change_consumer & cs = c;
      cs.add_dir(file_path("usr"), ea);
      cs.add_dir(file_path("usr/bin"), ea);
      cs.add_file(file_path("usr/bin/cat"),
                  file_id(hexenc<id>("adc83b19e793491b1c6ea0fd8b46cd9f32e592fc")),
                  ea);

      cs.add_dir(file_path("usr/local"), ea);
      cs.add_dir(file_path("usr/local/bin"), ea);
