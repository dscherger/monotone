...
COPT(branch_name, "branch", string, N_("select branch cert for operation"));
COPT(revision, "revision", string, N_("select revision id for operation"));
...
GOPT(help, "help", nil, N_("display help message"));
GOPT(version, "version", nil, N_("print version number, then exit"));
...
GOPT(db, "db", string, N_("passed: database location"));
