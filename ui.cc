...
static string
man_definition(vector<string> const & labels, string const & content)
{
  string out;
  out += ".IP \" " + (*labels.begin()) + "\"\n";
  if (labels.size() > 1)
...
static string
man_definition(string const & label, string const & content)
{
  vector<string> labels;
  labels.push_back(label);
  return man_definition(labels, content);
}
       ...
       iter != subcmds.end(); iter++)
    {
      ...
      if (!subcmd->is_leaf())
      ...
      else
        {
          ...
          string cmd_desc;
          cmd_desc += format_text(subcmd->desc()) + "\n";
          ...
          out += man_definition(cmd_calls, cmd_desc);
        }
    }
    return out;
}
...
CMD_HIDDEN(manpage, "manpage", "", CMD_REF(informative), "",
    N_("Dumps monotone's command tree in a txt2man compatible format"),
    "",
    options::opts::none)
{
  std::cout << man_title("monotone");
  std::cout << man_section(_("Name"));

  std::cout << _("monotone - a distributed version control system") << "\n";
  std::cout << man_section(_("Synopsis"));
  std::cout << man_bold(prog_name) << " "
            << man_italic(_("[options...] command [arguments...]"))
            << "\n";

  std::cout << man_section(_("Description"));
  std::cout << _("TODO: write a really fancy and catchy description") << "\n";

  std::cout << man_section(_("Global Options"));
  std::cout << get_options_string(options::opts::globals(), app.opts) << "\n";

  std::cout << man_section(_("Commands"));
  std::cout << get_command_tree(app.opts, CMD_REF(__root__));

  std::cout << man_section(_("See Also"));
  std::cout << (F("info %s and the documentation on <%s>")
                % prog_name % man_bold("http://monotone.ca/docs")).str() << "\n";

  std::cout << man_section("Bugs");
  std::cout << (F("Please report bugs to <%s>.")
                % man_bold("http://savannah.nongnu.org/bugs/?group=monotone")).str()<< "\n";

  std::cout << man_section("Authors");
  std::cout << (F("The monotone development team <%s>")
