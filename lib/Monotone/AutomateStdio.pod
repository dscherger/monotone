...
version (currently 0.41). If you happen to be using a newer version of Monotone
...
Please note that $mtn-E<gt>new() is simply an alias for
$mtn-E<gt>new_from_db().
...
deduced from the current workspace.  Please note that the workspace's base
directory may differ from that given to the constructor if the specified
workspace path is actually a subdirectory within that workspace.
...
(feature: MTN_IGNORE_SUSPEND_CERTS)
...
each one containing one or more of the following fields:
    ...
    (feature: MTN_INVENTORY_IN_IO_STANZA_FORMAT):
        ...
        birth_id - The revision id in which the node was first
                   ...
                   onwards. (feature: MTN_INVENTORY_INCLUDE_BIRTH_ID)
...
The $options argument is a list of valid options, with some having
arguments. For example, one could call this method specifying all of the
options like this (feature: MTN_INVENTORY_TAKE_OPTIONS):
...
or more of the following fields:
    ...
    conflict         - The nature of the conflict. Values can be one
                       ...
                       "directory_loop_created", "duplicate_name",
    ...
    left_type        - The type of conflict relating to the file or
                       ...
                       can be one of "added directory", "added file",
                       "deleted directory", "pivoted root",
                       "renamed directory" or "renamed file".
    ...
    right_file_id    - The id of the file in the right hand revision.
    right_name       - The name of the file or directory in the right
                       hand revision.
...
The $mtn-E<gt>get_content_changed() method is very slow since Monotone version
0.40. Hopefully this will be fixed in version 0.42.
