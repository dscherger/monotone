...
directory before running the mtn subprocess. The default action is to do so as
this is generally safer. This setting only affects objects constructed from
calling Monotone::AutomateStdio-E<gt>new() and
...
mtn from inside an application. This is why this class runs the mtn subprocess
