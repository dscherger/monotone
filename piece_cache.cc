...
namespace
{
  ...
  struct script_t
  {
    ...
    size_t memory_size()
    {
      return (extents.size() * sizeof(extent_t));
    }
  ...
  text_t::memory_size()
  {
    // FIXME: should we count the size of the text_t and file_id objects
    // here too?  and the shared_ptr counter?
  ...
  swap_in_script(file_id const & fid, database & db)
  {
    ...
    while (!(null_id(curr) || script_cache.exists(curr)))
      {
        shared_ptr<text_t> text = load_text(curr, db);
        curr = text->base;
        texts.append(text);
      }
    ...
    for (texts_t::reverse_iterator i = texts.rbegin(); i != texts.rend(); ++i)
      {
        ...
        // cache
        script_cache.insert(i->me, parse_script(*i));
      }
  ...
  parse_script(shared_ptr<text_t> const & text)
  {
    ...
    return shared_ptr<script_t>(new script_t(extents, precursors_memory_size));
  }


  // TODO:
  //   -- make script_t use a create() call like text_t and do accounting
  //      similarly
  //   -- more functions
