    ...
    options::opts::branch | options::opts::revision)
{
  ...
  if (app.opts.revision_selectors.size() == 0)
  ...
  else if (app.opts.revision_selectors.size() == 1)
    {
      ...
      complete(app.opts, app.lua, projects, idx(app.opts.revision_selectors, 0)(), revid);

      guess_branch(app.opts, projects, revid);
    ...
    | options::opts::exclude)
{
  ...
  if (app.opts.branchname().empty())
    {
      ...
      for (edge_map::iterator i = restricted_rev.edges.begin();
           ...
           i++)
        {
          ...
          guess_branch(app.opts, projects, edge_old_revision(i),
                       bn_candidate);
...

  {
    ...
    projects.get_revision_certs(restricted_rev_id, ctmp);
