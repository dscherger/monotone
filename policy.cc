...
policy_branch::init_lower()
{
  ...
  for (editable_policy::const_delegation_map::const_iterator i = dels.begin();
       i != dels.end(); ++i)
    {
      branch_name i_name(i->first, origin::internal);
      delegation_map::iterator j = old_delegations.find(i_name);
      ...
      if (!found)
        {
          delegations.insert(make_pair(i_name,
                                       create(*i->second, db)));
        }
    }
}
...
{
  cert_value const certval = typecast_vocab<cert_value>(br.uid);
                      ...
                      database & db)
{
  not_in_managed_branch p(db,
                          typecast_vocab<cert_value>(br.uid),
                          br.committers);
  return !p(rid);
}
				  ...
				  revision_id & rid)
{
  ...
  not_in_managed_branch p(db, typecast_vocab<cert_value>(name),
                          trusted_signers);
                        ...
                        policy_branch::branchmap & branchlist)
{
  ...
  for (editable_policy::const_branch_map::const_iterator i = br.begin();
       i != br.end(); ++i)
    {
      ...
      name.append(branch_name(i->first, origin::internal));
