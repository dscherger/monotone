...
editable_policy::tag::write(data & dat)
{
  ...
  dat = data(printer.buf, origin::internal);
}
      ...
      if (pa.symp(basic_io::syms::revision_id))
        {
          ...
          string rid;
          pa.hex(rid);
          rev = decode_hexenc_as<revision_id>(rid, dat.made_from);
        }
      else
        {
          E(false, dat.made_from, F("bad tag spec"));
        }
    }
  I(src.lookahead == EOF);
}
      ...
      if(pa.symp(basic_io::syms::branch_uid))
        {
          ...
          string branch;
          pa.str(branch);
          uid = branch_uid(branch, dat.made_from);
        }
        ...
        {
          E(false, dat.made_from, F("Unable to understand delegation"));
        }
    }
}
...
editable_policy::delegation::read(data const & dat)
{
  ...
  E(src.lookahead == EOF, dat.made_from, F("Bad delegation file"));

  E(seen_revid || seen_branchspec, dat.made_from,
                       ...
                       string const & name)
  {
    ...
    for (typename T::const_iterator i = cont.begin();
         i != cont.end(); ++i)
      {
        file_path old_path;
        if (!i->old_name.empty())
          old_path = dir / path_component(i->old_name, origin::internal);
        file_path new_path;
        if (!i->new_name.empty())
          {
            new_path = dir / path_component(i->new_name, origin::internal);
            if (!have_dir)
              {
                have_dir = true;
...
revision_id
editable_policy::commit(project_t & project,
                        key_store & keys,
                        ...
                        string author)
{
  ...
  project.put_standard_certs(keys, new_id,
                             uid, changelog, date_t::now(), author);
