...
database::info(ostream & out)
{
  ...
  counts.push_back(imp->count("sentinels"));
  ...
  vector<string> bytes;
  {
    ...
    bytes.push_back(imp->space("sentinels", "length(id) + length(data)", total));
...
database::delete_existing_revs_and_certs()
{
  ...
  imp->execute(query("DELETE FROM sentinels"));
