...
=head1 SYNOPSIS
  ...
  use Monotone::AutomateStdio qw(:constants);
  my(@manifest,
     $mtn,
     @revs);
...
Methods that return data do so via their first argument. This argument is a
reference to either a scalar or a list depending upon whether the data returned
by the method is raw data or a list of items respectively. Methods that return
lists of records also provide the option of returning the data as one raw chunk
