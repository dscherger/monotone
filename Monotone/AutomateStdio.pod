...
containing the following fields:
    ...
    status       - The three inventory status characters for the file
                   or directory.
...
Whilst the object does know what version of mtn automate interface it is
dealing with it currently does nothing with that knowledge. However, hopefully
as usage of this library grows then it should not be too difficult to code in
exceptions to cleanly deal with differences.
