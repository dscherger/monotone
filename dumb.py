...
#                        print stanza_type, ":", new_manifest
                    elif stanza_type == "old_revision":
                        if not old_manifest:
                            old_manifest = stanza[0][1]
#                            print stanza_type, ":", old_manifest
...
#                        print stanza_type, ":", stanza[1][1],":", stanza[2][1]
                ...
                if old_manifest:
                    mdp = monotone.get_manifest_delta_packet(old_manifest,
                                                                   new_manifest)
#                    print "manifest_delta:",mdp
                    rdata.write(mdp)
                else:
                    mpp=monotone.get_manifest_packet(new_manifest)
#                    print "manifest_packet:",mpp
                    rdata.write(mpp)
                for new_fid, old_fid in new_files.items():
                    if old_fid:
#                        print "get_file_delta:",old_fid, new_fid
                        fdp =monotone.get_file_delta_packet(old_fid, new_fid)
#                        print "file_delta:",fdp
                        rdata.write(fdp)
                    else:
