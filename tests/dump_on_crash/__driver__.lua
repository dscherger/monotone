...
check(remove("_MTN"))
check(mtn_no_ws("crash", "I"), 3, false, false)
check(exists("dump"))

mkdir("try")
check(mtn_no_ws("crash", "I", "--confdir=try"), 3, false, false)
check(exists("try/dump"))

check(mtn_no_ws("crash", "I", "--dump=fork"), 3, false, false)
check(exists("fork"))
...
for _,tag in pairs({  'std::bad_cast',
		      ...
		      'std::exception' }) do
   remove("fork")
   check(mtn_no_ws("crash", tag, "--dump=fork"), 3, false, false)
   check(exists("fork"))
end

-- bad_alloc is special
remove("fork")
check(mtn_no_ws("crash", "std::bad_alloc", "--dump=fork"), 1, false, false)
...
for _,tag in pairs({ 3, 6, 11 }) do
   check(mtn_no_ws("crash", tag, "--dump=fork"), -tag, false, false)
   check(not exists("fork"))
end
