...
check(mtn("commit", "--message-file=msgfile.txt"), 0, false, false)
...
check(indir("alt_wrk", mtn("branch", "outbranch")), 0, false, false)
check(indir("alt_wrk", mtn("commit", "--message-file=../message-out-of-copy.txt")), 0, false, false)
...
check(mtn("commit", "--message-file=to-be-or-not-to-be.txt"), 1, false, false)

----------------------
--then verify that --message and --message-file together cause an error
----------------------

check(mtn("commit", "--message-file=msgfile.txt",
...
check(mtn("--rcfile=commit_log.lua", "commit",
