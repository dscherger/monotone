...
public class Log2Gxl extends Thread {
     ...
     * @throws IllegalStateException if the header lines aren't as expected     
     */
    private void parseDeletedEntries() throws IOException,IllegalStateException {
        String entries=readFileBlock("Deleted entries:");
        if(!simpleGXL) currentNode.setAttr("Deleted entries",new GXLString(entries));
    }
    ...
    /**
     * Parse the added directories section of a log entry from a monotone log output
     * Optionally pass the list of files to GXL (Don't do this is you're using dot as dxl2dot chokes on it)
     * @throws IOException if there is a read error on the input stream or the input stream runs dry
     * @throws IllegalStateException if the header lines aren't as expected     
     */
    private void parseAddedDirectories() throws IOException,IllegalStateException {
        String dirs=readFileBlock("Added directories:");
        if(!simpleGXL) currentNode.setAttr("Added directories",new GXLString(dirs));
    }

    /**
     * Parse the renamed files section of a log entry from a monotone log output
     * Optionally pass the list of files to GXL (Don't do this is you're using dot as dxl2dot chokes on it)
     * @throws IOException if there is a read error on the input stream or the input stream runs dry
     * @throws IllegalStateException if the header lines aren't as expected     
     */
    private void parseRenamedEntries() throws IOException,IllegalStateException {
        String entries=readFileBlock("Renamed entries:");
        if(!simpleGXL) currentNode.setAttr("Renamed entries",new GXLString(entries));
    }
     ...
     * @throws IllegalStateException if the header lines aren't as expected     
     */
    private void parseModifiedAttrs() throws IOException,IllegalStateException {
        String attrs=readFileBlock("Modified attrs:");
        if(!simpleGXL) currentNode.setAttr("Modified attrs",new GXLString(attrs));
    }
    ...
    private void parseFiles() throws IOException,IllegalStateException {
        ...
        if(line.length()>0) throw new IOException(source.getLineNumber()+": Unexpected data ["+line+"]");
        line=lookahead();
        if(line.startsWith("Deleted entries:")) parseDeletedEntries();
        line=lookahead();
        if(line.startsWith("Renamed entries:")) parseRenamedEntries();
        line=lookahead();
        if(line.startsWith("Renamed directories:")) parseRenamedDirectories();
        line=lookahead();
        if(line.startsWith("Added files:")) parseAddedFiles();
        line=lookahead(); 
        if(line.startsWith("Added directories:")) parseAddedDirectories();
        line=lookahead(); 
        if(line.startsWith("Modified files:")) parseModifiedFiles();
        line=lookahead(); 
        if(line.startsWith("Modified attrs:")) parseModifiedAttrs();
    }
    ...
    private void start(String argv[]) throws IOException,IllegalStateException {
        start(argv,System.in,System.out);
    }
