  ...
  void forward_or_cross_edge(Edge e)
    {
      ...
      if (a_has_branch && b_has_branch)
      ...
      else
        {
          ...
          // the streamliner will handle this case later on.
        }
    }
...
public:
  streamliner(cvs_history & c, int & edges_removed)
    : cvs(c),
      edges_removed(edges_removed)
    { }
  ...
  void tree_edge(Edge e)
    {
#ifdef DEBUG_BLOB_SPLITTER
      L(FL("streamliner: tree edge: %d -> %d") % e.first % e.second);
#endif
    }
  ...
  void forward_or_cross_edge(Edge e)
    {
#ifdef DEBUG_BLOB_SPLITTER
      L(FL("streamliner: cross edge: %d -> %d") % e.first % e.second);
#endif
  ...
  void back_edge(Edge e)
    {
      ...
      L(FL("streamliner: back edge from blob %d (%s) to blob %d (%s) - ABORTING!")
        % e.first % get_event_repr(cvs, *cvs.blobs[e.first].begin())
        % e.second % get_event_repr(cvs, *cvs.blobs[e.second].begin()));
  ...
  while (1)
    {
      int edges_removed = 0;
      cvs.import_order.clear();
      streamliner vis(cvs, edges_removed);
      cvs.depth_first_search(vis, back_inserter(cvs.import_order));
